...
parseExpression = head . map fst . concat . map (parse (successful parse_expr)) . splalex
...
parseTypeAliasDecl :: String -> TypeAliasDecl
parseTypeAliasDecl = head . map fst . concat . map (parse $ successful parse_type_decl) . splalex
